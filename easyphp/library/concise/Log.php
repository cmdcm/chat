...
class Log
{
	...
	public function __construct (Request $request)
	{
		...
		$this->stream  = new StreamHandler($this->parseLogPath(EASY_RUMTIME_PATH . '/log'), Logger::DEBUG);
	 ...
	 * @return string
	 */
	public function parseLogPath ($basePath = '')
	{		
		is_dir($basePath) || mkdir($basePath);
		$path    = $basePath . '/' . $this->request->module();
		is_dir($path) || mkdir($path);
		$format  = $this->config['format'];
		$path = $path . '/' . $this->channel .'_' .$format() . '.' . $this->config['ext'];
	...
	{
		$end = microtime(true);
		return round($end - EASY_PHP_START,3);
	}
	/**
	 * 返回Monlog对象
	 * @return object
